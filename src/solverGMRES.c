...
{
    FILE *fin;
    unsigned int i, iter1 = 0, iter2 = 0, j, convergence = 1;
    ...
    fprintf(fg,"\n%14s%25s\n", "ITERATION NO.", "RELATIVE RESIDUAL");
    fprintf(fg,"%14s%25s\n", "***************", "*******************");
    fclose(fg); fg = fopen("gmres.log","a");
    ...
    for(i = 1; i <= MAX_OUTER_LOOP; i++){
        ...
        convergence = iterGMRES(size,iter1,&iter2,tolerance,A,x,B);
