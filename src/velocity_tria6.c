...
#include "constants.h"
#include "velocity_tria6.h"

int velocity_tria6(double *Xin, double **mNodes, unsigned int **mElems, 
                   double *vProbParam, double *vB, double *U)
{
    const unsigned int ELEMS = nElems, NODES_IN_ELEM = 6;
    unsigned int currentNode, i, j, SinNode, xNode, yNode, zNode; 
    ...
    for(i = 0; i < ELEMS; i++){
        ...
        if(SinNode == 0) intGStk_tria6(X,Xin,Int);
        else intSingularGStk_tria6(SinNode,X,Xin,Int);
        ...
        for(j = 0; j < NODES_IN_ELEM; j++){
            ...
            yNode = xNode + nNodes;
            zNode = yNode + nNodes;
            U[0] += Int[0][0][j]*vB[xNode] + Int[0][1][j]*vB[yNode] + Int[0][2][j]*vB[zNode];
            U[1] += Int[1][0][j]*vB[xNode] + Int[1][1][j]*vB[yNode] + Int[1][2][j]*vB[zNode];
            U[2] += Int[2][0][j]*vB[xNode] + Int[2][1][j]*vB[yNode] + Int[2][2][j]*vB[zNode];
        }
    }
    U[0] = U[0]*factor;
    U[1] = U[1]*factor;
