...
#include "constants.h"
#include "pressure_tria6.h"

double pressure_tria6(double *Xin, double **mNodes, unsigned int **mElems, 
                     double *vB)
{
    const unsigned int  ELEMS = nElems, NODES_IN_ELEM = 6;
    unsigned int    currentNode, i, j, SinNode, test, xNode, yNode, zNode; 
    double   dx, dy, dz, P;
    double   X[6][3], Int[3][6];
    ...
    for(i = 0; i < ELEMS; i++){
        ...
        if(test == 0) intHStk_tria6(X,Xin,Int);
        else intSingularHStk_tria6(SinNode,X,Xin,Int);
